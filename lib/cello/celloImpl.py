...
class cello:
    ...
    def run_cello(self, ctx, params):
        ...
        if html_init_dict["png_files_found"]:
            png_file_links = []
            png_files = html_init_dict['png_files']
            for i in range(len(png_files)):
                    png_file_dict = {"path":png_files[i], "name": "png_file_" + str(i+1), "label": "Truth RPU diagram " + str(i+1)}
                    png_file_links.append(png_file_dict)

        if html_init_dict["wiring_diagram_found"] and html_init_dict["png_files_found"]:
            ext_report_params['html_links'] = wiring_diagram_links + png_file_links
            ...
            ext_report_params["direct_html_link_index"] = 0
        elif html_init_dict["png_files_found"]:
            #Note, there may be an issue with having the html link be a png file.
            ext_report_params['html_links'] = png_file_links
            ext_report_params["direct_html_link_index"] = 0
