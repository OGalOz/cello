...
def make_kbase_genomes(output_files, kb_output_folder, output_folder, gfu, 
        ws_name, main_output_name, config_info):
            ...
            for ape_fp in ape_files:
                ...
                genb_gen_dict = make_genbank_genome_dict(gbk_file_name, g_name, 
                        ws_name)
...
def turn_ape_to_gbk(output_files, kb_output_folder, output_folder, config_info):
    ...
    for out_f in output_files:
        if out_f[-4:] == ".ape":
            if "plasmid_circuit" in out_f:
                ...
                circuit_ape_files.append(os.path.join(kb_output_folder, 
                    os.path.join(output_folder, out_f)))
            elif "plasmid_output" in out_f:
                logging.info("Recognized plasmid_output .ape file: " + out_f)
                output_ape_files.append(os.path.join(kb_output_folder, 
                    os.path.join(output_folder, out_f)))
            elif "sensor" in out_f:
                logging.info("Recognized sensor .ape file: " + out_f)
                sensor_ape_files.append(os.path.join(kb_output_folder, 
                    os.path.join(output_folder, out_f)))
            else:
                logging.info("Other .ape file: " + out_f)
                extra_ape_files.append(os.path.join(kb_output_folder, 
                    os.path.join(output_folder, out_f)))
    ...
    for ap_f in ape_files:
        ...
        if response != 0:
            ...
            logging.critical("Issue with replacing 'label' with 'locus_tag'. \
                    Could be that no 'label's exist.")
        #Placeholder genome name:
...
def replace_plasmid_sections_with_features(plasmid_filename, config_info):
    ...
    if "circuit" in plasmid_filename:
        ...
        detailed_plasmid_insertion(plasmid_filename, config_info["circuit_base"], 
                config_info["circuit_insertion_bp"])

    elif "output" in plasmid_filename:
        logging.info("Recognized output plasmid for replacing sections")

        detailed_plasmid_insertion(plasmid_filename, config_info["output_base"],
                config_info["output_insertion_bp"])
    elif "sensor" in plasmid_filename:
        logging.info("Recognized sensor plasmid for replacing sections")

        detailed_plasmid_insertion(plasmid_filename, config_info["sensor_base"],
                config_info["sensor_insertion_bp"])
    else:
